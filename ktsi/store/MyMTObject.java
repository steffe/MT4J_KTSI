...
public class MyMTObject extends MTRoundRectangle {
	...
	public MyMTObject(final MTApplication pApplet2, final ModelMtObjects model, final int objectID) {
		...
		attributslist = new ArrayList<Integer>();
				
		this.model = model;
		
		for (Iterator<ModelMtAttributs> it = model.getObjectattributs().iterator(); it.hasNext();) {
			attributslist.add(it.next().getId());
			/*
			attributslist.add(1);
			attributslist.add(0);
			attributslist.add(2);
	...
	int difx = 40;
		for (Integer it : attributslist) {
			...
			case(0):
				...
				myAttributs.add(new MTTextAttribut(pApplet, fontArialMini, 250, 30, false, model.getObjectattributs().get(0).getAttributcontent().get(0).getValue(), model.getObjectattributs().get(0).getLable(), labelfont));
			...
			case(2):
				...
				myAttributs.add(new MTDropDownList(pApplet, fontArialMini, 250, 30, model.getObjectattributs().get(0).getAttributcontent().get(0).getValue(), labelfont));
				myAttributs.get(i).setPositionRelativeToParent(new Vector3D(this.getWidthXY(TransformSpace.LOCAL) / 2, x));
