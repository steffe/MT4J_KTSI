...
//		initTiles();
	...
	private void init(PImage p, int horizontalTileCount, int verticalTileCount){
		...
		this.setTileWidth((float)p.width/(float)horizontalTileCount);
		this.setTileHeight((float)p.height/(float)verticalTileCount); 
		System.out.println("In puzzle Factory: title width "+(float)getTileWidth()+" title height "+(float)getTileHeight());
		initTiles();
	}
	...
	private void initTiles(){
		...
		Vertex.scaleVectorArray(upDownOrderVerticalRightOut, Vector3D.ZERO_VECTOR, (1f/origHeight) * getTileHeight(), (1f/origHeight) * getTileHeight(), 1);
...
//		getCanvas().addChild(p2);
		...
		Vertex.scaleVectorArray(leftRightHorizontalUpOut, Vector3D.ZERO_VECTOR, (1f/origHeight) * getTileWidth(), (1f/origHeight) * getTileWidth(), 1);
...
//		getCanvas().addChild(p4);
	...
	public AbstractShape[] createTiles(PImage p, int horizontalTileCount, int verticalTileCount){
		...
		System.out.println("Printing verTileCount in PF "+verticalTileCount);
		System.out.println("Printing horTileCount in PF "+horizontalTileCount);
...
//						left = getFittingTileSideTo(getRightOfLeftTile(tiles, i, j));
				...
				MTComplexPolygon tile = getPolygon(app, top, right, bottom, left, this.getTileWidth(), this.getTileHeight());
				tile.setName(i + "" + j);
...
//						vertex.setTexCoordV((vertex.y - upperLeftY + (i * tileHeight)) / p.height);
                        ...
                        vertex.setTexCoordU((vertex.x + (j * getTileWidth())) / p.width);
                        vertex.setTexCoordV((vertex.y + (i * getTileHeight())) / p.height);
...
//	}
	...
	private Vertex[] getTile(TileSide top, TileSide right, TileSide bottom, TileSide left){
		...
		case linear:
			...
			list.add(new Vertex(getTileWidth(), 0));
			break;
		case pinIn:
...
//			list.add(new Vertex(tileWidth,0));
			list.add(new Vertex(getTileWidth(), getTileHeight()));
			break;
		case pinIn:
			addAll(getCopyOffset(this.upDownOrderVerticalLeftOut, getTileWidth(), 0), list);
			break;
		case pinOut:
			addAll(getCopyOffset(this.upDownOrderVerticalRightOut, getTileWidth(), 0), list);
			break;
		default:
			break;
		}
...
//			list.add(new Vertex(tileWidth, tileHeight));
			list.add(new Vertex(0, getTileHeight()));
			break;
		case pinIn:
			addAll(getCopyOffset(this.rightLeftHorizontalUpOut, 0, getTileHeight()), list);
			break;
		case pinOut:
			addAll(getCopyOffset(this.rightLeftOrderHorizontalDownOut, 0, getTileHeight()), list);
			break;
		default:
			break;
		}
